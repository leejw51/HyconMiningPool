...
export class MinerServer {
    ...
    private candidate(previousDBBlock: DBBlock, previousHash: Hash): void {
        ...
        console.log(`MinerWallet ${globalOptions.minerAddress}`)
    ...
    public async sendReward(wallet: string, amount: number, fee: number) {
        logger.info(`SendReward To ${wallet}  Amount ${amount}  Fee ${fee}`)
    }
}
